...
namespace Infrastructure.Specifications
{
    public class InvoiceSpecs : BaseSpecification<InvoiceMaster>
    {
        public InvoiceSpecs(List<DateTime?> docDate, List<DateTime?> dueDate,
            List<DocumentStatus?> states, TransactionFormFilter filter, bool isTotalRecord) : base(x => (docDate.Count() > 0 ? docDate.Contains(x.InvoiceDate) : true) && (dueDate.Count() > 0 ? dueDate.Contains(x.DueDate) : true)
			...
			&& (x.InvoiceDate >= (filter.StartDate != null ? filter.StartDate : x.InvoiceDate) && x.InvoiceDate <= (filter.EndDate != null ? filter.EndDate : x.InvoiceDate))
